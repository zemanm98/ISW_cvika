...
class DeepQLearningAgent(Agent):
    ...
    def best_action(self, state, valid_actions):
        if random.uniform(0, 1) < self.epsilon:
            ...
            out = self.Q_nn(torch.from_numpy(state.astype(np.double)))
    ...
    def add_to_candidate(self):
        ...
        reward = torch.from_numpy(np.array([sampled[i][3] for i in range(0, len(sampled))]))
        state = torch.from_numpy(np.array([sampled[i][0] for i in range(0, len(sampled))]))
        state2 = torch.from_numpy(np.array([sampled[i][4] for i in range(0, len(sampled))]))
        ...
        loss = self.l1(state_action_values, expected_state_action_values)
        # print(loss)
